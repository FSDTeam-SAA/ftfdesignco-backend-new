...
const adminResetPassword = async (
  ...
  const adminUser = await User.isUserExistByEmail(adminEmail)
  if (!adminUser) {
    throw new AppError(
      'No account found with the provided credentials.',
      StatusCodes.NOT_FOUND,
  ...
  if (!users.length) {
    throw new AppError('No users found to reset', StatusCodes.NOT_FOUND)
  }
  ...
  const hashedPassword = await bcrypt.hash(
    config.adminDefaultResetPassword,
    Number(config.bcryptSaltRounds),
  )
  ...
  return {
    ...
    defaultPassword: config.adminDefaultResetPassword,
