...
const rigionProducts = catchAsync(async (req, res) => {
  const page = Number(req.query.page) || 1
  const limit = Number(req.query.limit) || 10
  const productRigion = req.query.rigion as string
  const result = await productService.getRigionProducts(
    page,
    limit,
    productRigion,
  )
